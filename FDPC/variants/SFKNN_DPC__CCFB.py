...
class SFKNN_DPC:
    def __init__(self, X, f_attr_vals, k=5, cluster_num=5,):
        self.k = k
...
all_datasets = ['abalone']
# 'abalone', 'dermatology', 'hcvdat0',
#                 'obesity', 'Wholesale', 'Rice', 'adult', 'bank',
...
for data in all_datasets:
    for cluster_num in range(4, 5):
        for k in range(5, 6):
            ...
            model = SFKNN_DPC(cluster_attr_vals, f_attr_vals=f_attr_vals,
                              k=k, cluster_num=cluster_num)
            model.fit()
            # 可视化聚类结果
            model.visualize_clusters(cluster_attr_vals, title=data)
            # model.evaluate(data, k)
# 使用Iris数据集进行测试

"""
类定义
# class KS_FDPC_Clustering:
#     def __init__(self, X, f_attr_vals, k=3, dc=0.5):
#         self.k = k
#         self.dataset_cluster = X
#         self.dist_matrix = pairwise_distances(X)
#         self.dc = get_varience.select_dc_orin(self, dc)
#         # 每个点的敏感属性的取值（）
#         self.fair_attr_vals = f_attr_vals
#         # 各个敏感属性值在数据集中的占比
#         self.fattr_val_ratio = get_varience.cal_fair_attr_val_ratio(self)
...
#         for i, label_id in enumerate(unique_group_label):
#             if label_id == -1:
#                 plt.scatter(X[self.group_label == label_id, 0], X[self.group_label == label_id, 1], c='black', s=7, label='Noise')
#             else:
#                 color = colors[i % len(colors)]
#                 plt.scatter(X[self.group_label == label_id, 0], X[self.group_label == label_id, 1], c=color, s=7,
#                             label=f'Cluster {label_id}')
...
"""
data_path = 'D:\\code\\python\\pythoncode\\machineLearning' \
            '\\cluster\\DensityCluster\\DPC\\data_process\\data\\fairness_data\\'
...
#     random_dataset = []  # 'adult', 'athlete', 'bank']
# 示例数据
cluster_attr_vals, f_attr_vals, f_attr = read_data('abalone')
...
"""
评估
 def evaluate(self, dc, data_name):
        # Calculate evaluations
        self.dc = dc
        self.fair_loss = np.sum(cal_dc_fair_loss(self, dc))
        self.balance_val = balance(self)
        self.sc = compute_SC(self)

        # Log to Excel
        self.log_to_excel(data_name)

    def log_to_excel(self, data_name):
        # Prepare data for Excel log
        data = {
            'Data Name': [data_name],
            'Cluster Count': [self.k],
            'DC': [self.dc],
            'SC Value': [self.sc],
            'Balance': [self.balance_val],
            'DC Fair Loss': [self.fair_loss]
        }

        # Create or append to the Excel file
        excel_file = f"{data_name}_results.xlsx"
        try:
            df = pd.read_excel(excel_file)
            df = df.append(pd.DataFrame(data), ignore_index=True)
        except FileNotFoundError:

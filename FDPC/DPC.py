...
class DPC:
    def __init__(self, data_name, dataset, cluster_num=None):

        self.dataset_cluster = dataset
        self.data_name = data_name
        self.dist_matrix = self.getDistanceMatrix()
        self.dc = self.select_dc()
        self.rho_matrix = self.get_local_density(method="Gaussion")
        ...
        # self.group_label = self.classity_by_dc()
    ...
    def find_centers_auto(self, k=None):
        ...
        if k == 0 or k == None:
            ...
            rho_delta = self.rho_matrix * self.deltas
